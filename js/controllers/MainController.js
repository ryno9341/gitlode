(function () {
  ...
  function MainController(
    ...
    function init() {
      ...
      if (params.url) ctrl.repoUrl = params.url;
    ...
    function startDownload() {
      ...
      if (!repoInfo) {
        ...
        ctrl.errorMessage =
          'Please provide a valid repository or directory link.';
        ctrl.isLoading = false;
        return;
      }
      addLogMessage(
        `Parsed URL: /${repoInfo.owner}/${repoInfo.repo}/${repoInfo.branch}/${repoInfo.path}`,
        'info'
      );
      ...
      GitHubService.getDirectoryFiles(contentsUrl, ctrl.githubToken)
        ...
        .then(function (zip) {
          ...
          ctrl.progress = 100;
          const zipFileName = `${repo}-${
            path.replace(/\//g, '_') || branch
          }.zip`;
        ...
        .catch(function (error) {
          ...
          if (error.status === 404)
            message = 'Repository, branch, or directory not found.';
